...
from .transformer_tools import to_2tuple, to_ntuple, DropPath
...
class S2WAT(nn.Module):
  ...
  def forward_features(self, x):
    ...
    # --- 修改開始 ---
    out_features = []  # 建立一個空列表來收集各層特徵
    for layer in self.layers:
      x = layer(x)
      out_features.append(x)  # 將每一層(淺、中、深)的輸出都裝進列表
      # print(x[0].shape)

    return out_features  # 回傳包含所有層次特徵的列表
